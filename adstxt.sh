...
Nregistro=$(curl -s $url | awk -F',' '!/^#/ {print $1 $2 $3 $4}' | wc | awk '{print $1}')
NregistroFiltrada=$(curl -s $url | awk -F',' '!/^#/ {print $1 $2 $3 $4}' | grep "$2" | wc | awk '{print $1}')
...
then
	...
	then
		...
		echo -e "-----------------------------------"
		curl -s $url | awk -F',' '!/^#/ {print $1 "\t" $2 "\t" $3 "\t" $4}' | grep "$2" | column -t -N Dominio:,ID-Publisher:,Relación:,ID-Plataforma: -s $'\t'
	else
		echo -e "${CTurquesa}[+]${CFin} ${CTurquesa}Nº de registros: \t $Nregistro ${CFin}"
		echo -e "-----------------------------------"
		curl -s $url | awk -F',' '!/^#/ {print $1 "\t" $2 "\t" $3 "\t" $4 "\n----"}' | column -t -N Dominio:,ID-Publisher:,Relación:,ID-Plataforma: -s $'\t'
	fi
else
